...
begin {
    ...
    [string]$fLogContentFile = "$($Env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\helloWorld-$($doTest).log"
    #region :: functions
...
process {
    ...
    if ($runScriptIn64bitPowerShell -eq $true -and $([System.Environment]::Is64BitProcess) -eq $false) {
        ...
        Write-Error -Message "Windows PowerShell 64-bit is requered." -Category "ResourceUnavailable" -ErrorId "B001" -Category "ResourceUnavailable" -ErrorId "01"
        exit 1
    }
    if ([bool]$runScriptAsUser -eq $true -and $env:username -eq "SYSTEM") {
        Write-Output -InputObject "Script must be run in USER context."
        Write-Error -Message "Script must be run in USER context." -Category "ResourceUnavailable" -ErrorId "02"
        exit 1
    }
    #endregion
    try {
        ...
        switch ($doTest) {
            ...
            11 {
                ...
                try {
                    fLogContent -fLogContent "## Hello world sample script for exploring remediation scripts" -fLogContentComponent ""
                    fLogContent -fLogContent "Description: Hello world sample script for exploring remediation functionality" -fLogContentComponent ""
