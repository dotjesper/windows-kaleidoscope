...
# Microsoft Intune remediation: Collect Windows System Stability Index
...
A quick way to see the state of a Windows machine is to open Reliability Monitor. It is a built-in tool that has been around since Windows Vista and still works in Windows 10. You probably do not hear much about it, but it is quite useful.
...
Name: **Collect Windows System Stability Index**

Description: **Collect Windows System Stability Index, fails if Reliability Stability Metrics average is below 4.**
...
| Exit code | Output |
|:---------:|:-------|
| `exit 0` | `[00.000] Reliability index is above the index threshold (Avr 7.5 Max 9.2 Min 5.1)` |
| `exit 1` | `[00.000] Reliability index is below the index threshold (Avr 2.3 Max 4.1 Min 1.0)` |
...
Get-CimInstance -ClassName Win32_ReliabilityStabilityMetrics | Select-Object StartMeasurementDate, SystemStabilityIndex | Out-GridView
```
