...
process {
    ...
    try {
        ...
        if ($regValues.KfmFoldersProtectedNow -eq $regValues.KfmFoldersProtectedOnce) {
            ...
            if ([string]::IsNullOrEmpty($($regValues.KfmFoldersProtectedOnce))) {
                Write-Output -InputObject "OneDrive Known Folder Move for Business1 (KfmFoldersProtectedOnce) is empty."
                exit 0
            }
            else {
                Write-Output -InputObject "OneDrive Known Folder Move for Business1 folder has failed [ $($regValues.KfmFoldersProtectedNow) | $($regValues.KfmFoldersProtectedOnce) ]."
                exit 1
            }
        }
    }
