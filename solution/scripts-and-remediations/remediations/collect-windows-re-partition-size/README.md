...
The Windows RE tools should be in a partition that is separate from the Windows partition. This separation supports automatic failover and the startup of partitions that are encrypted by using Windows BitLocker Drive Encryption.
...
| Exit code | Output |
|:---------:|:-------|
| `exit 0` | `[00.000] Windows RE partition size is above the threshold [980MB > 900MB] [0,4].` |
| `exit 1` | `[00.000] Windows RE partition size is below the threshold [450MB < 900MB] [0,4].` |
| `exit 1` | `[00.000] Windows RE is disabled.` |
| `exit 1` | `[00.000] Windows RE location not found.` |
