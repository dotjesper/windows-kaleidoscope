...
.DESCRIPTION
    ...
    If the service does not exist, the remidation package will attempt to add Windows Sense Client Capability feature.
...
process {
    ...
    try {
        ...
        if ($WindowsSenseService) {
            Write-Output -InputObject "Windows Sense service is present [$($WindowsSenseService.Status)]"
            exit 0
        }
        else {
            ...
            if ($getRebootReasonID -eq $rebootReasonId) {
                Write-Output -InputObject "Windows Sense service not found - $rebootReason"
                exit 0
            }
            else {
                Write-Output -InputObject "Windows Sense service not found"
                exit 1
            }
            #endregion
        }
