    ...
    try {
        ...
        if (Test-Path -Path $($regRoot + ":\" + $regPath)) {
            ...
            if (($regValues.LegalNoticeCaption.Length -gt 0) -or ($regValues.LegalNoticeText.Length -gt 0)) {
                ...
                Write-Output -InputObject "Interactive Logon Message configurations found."
                exit 1
            }
            else {
                Write-Output -InputObject "Interactive Logon Message values not configured."
                exit 0
            }
        }
        else {
            Write-Output -InputObject "Interactive Logon Message configurations not found."
            exit 0
        }
    }
    catch {
